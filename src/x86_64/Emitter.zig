...
pub fn emit(p: AIR, writer: *Writer) Error!void {
    var self: Emitter = .{ .writer = writer };
    try self.program(p);
    try self.writer.flush();
}

fn program(self: Emitter, p: AIR) Error!void {
...
fn subroutine(self: Emitter, s: AIR.Subroutine) Error!void {
    try self.printLine(
        \\.global {s}
        \\{s}:
    , .{ s.name, s.name });
...
fn instruction(self: Emitter, instr: AIR.Instruction) Error!void {
    switch (instr) {
...
fn operandFmt(self: AIR.Operand, writer: *Writer) Error!void {
    switch (self) {
...
fn operand(op: AIR.Operand) std.fmt.Alt(AIR.Operand, operandFmt) {
    return .{ .data = op };
}
